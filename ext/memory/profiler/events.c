...
struct Memory_Profiler_Events {
	...
	// Single postponed job handle for processing the global queue
...
// Creates a TypedData object - caller should pin it
static VALUE Memory_Profiler_Events_new(void) {
	// Allocate the Events structure wrapped in TypedData
	...
	// Pre-register the single postponed job for processing the global queue
	events->postponed_job_handle = rb_postponed_job_preregister(
		0, // flags
		Memory_Profiler_Events_process_queue,
		(void *)events  // Pass the events struct as arg
	);
...
static void Memory_Profiler_Events_process_queue(void *arg) {
	...
	for (size_t i = 0; i < events->queue.count; i++) {
		...
		// This ensures we continue processing even if a callback raises
		int state = 0;
		rb_protect(Memory_Profiler_Events_process_event_protected, (VALUE)event, &state);
		
		if (state) {
			// Exception occurred - log it if debugging
			// rb_warn("Exception in event processing callback (caught and suppressed)");
			rb_set_errinfo(Qnil);
		}
	}
	
	// Always clear the global queue, even if exceptions occurred
	// This is critical to prevent stale object references
